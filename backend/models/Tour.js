import mongoose from 'mongoose';

const TourSchema = new mongoose.Schema({
  title: { type: String, required: true },
  slug: { type: String, unique: true, sparse: true },
  subtitle: String,
  location: String,
  description: String,

  basePrice: { type: Number, required: true },
  kidsPrice: { type: Number, default: 0 },
  rating: { type: Number, default: 4.5 },
  nights: { type: Number, default: 3 },
  featured: { type: Boolean, default: false },

  tags: { type: [String], default: [] },

  images: { type: [String], default: [] },
  img: { type: String },

  mapEmbedUrl: String,

  // --- NEW SEO SECTION ---
  seo: {
    title: String,
    description: String,
    keywords: String,
  },
  // -----------------------

  pricing: {
    mealPerPerson: { type: Number, default: 450 },
    teaPerPerson: { type: Number, default: 60 },
    bonfire: { type: Number, default: 500 },
    tourGuide: { type: Number, default: 1000 },
    comfortSeat: { type: Number, default: 800 },
    sharingPrice: { type: Number, default: 400 },

    room: {
      standard: { type: Number, default: 1500 },
      panoramic: { type: Number, default: 2500 },
    },
    personalCab: {
      rate: { type: Number, default: 3200 },
      capacity: { type: Number, default: 4 },
    },
    tourManagerFee: { type: Number, default: 5000 },
  },

  inclusions: { type: [String], default: [] },
  itinerary: [
    {
      day: Number,
      title: String,
      details: String,
      stay: String,
      breakfast: { text: String, override: { type: Boolean, default: false } },
      lunch: { text: String, override: { type: Boolean, default: false } },
      dinner: { text: String, override: { type: Boolean, default: false } }
    }
  ],
  faqs: [{ q: String, a: String }],

  reviews: [{
    title: String,
    name: String,
    email: String,
    mobile: String,
    rating: Number,
    text: String,
    date: String
  }]

}, { timestamps: true });

export default mongoose.models.Tour || mongoose.model('Tour', TourSchema);