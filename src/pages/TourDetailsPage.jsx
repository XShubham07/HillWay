...
import {
  ...
  FaPenNib, FaEnvelope, FaExclamationTriangle, FaPlus, FaMinus, FaBus, FaIdCard
} from "react-icons/fa";
...
// --- HELPER: Extract meals ---
const getMealsForDay = (day) => {
  const meals = [];
  if (day.breakfast?.text && day.breakfast.text.trim()) meals.push(day.breakfast.text);
  if (day.lunch?.text && day.lunch.text.trim()) meals.push(day.lunch.text);
  if (day.dinner?.text && day.dinner.text.trim()) meals.push(day.dinner.text);
  return meals;
};
...
export default function TourDetailsPage() {
  ...
  const [globalNotes, setGlobalNotes] = useState({ stayNote: '', foodNote: '', transportNote: '' });
  ...
  const displayPrice = useMemo(() => {
    if (!tour) return 0;
    const hotelPricePerNight = tour.pricing?.room?.standard || globalPricing.standardRoomPrice;
    const nights = tour.nights || 1;
    return Math.round(tour.basePrice + ((hotelPricePerNight * nights) / 3));
  }, [tour, globalPricing]);
    ...
    fetch(`https://admin.hillway.in/api/tours/${id}`, { signal: controller.signal })
      ...
      .then(tourData => {
        if (tourData.success) {
          ...
          // NEW: SET SEO DATA FROM TOUR
          setSeoData({
            title: t.seo?.title || `${t.title} - HillWay`,
            description: t.seo?.description || t.subtitle || `Explore ${t.title} with HillWay.`,
            keywords: t.seo?.keywords || t.tags?.join(", ") || "travel, sikkim, tours",
            image: t.images?.[0] || '/placeholder.jpg'
          });
        }
      })
      .catch(err => {
      ...
      .then(pricingData => {
        if (pricingData.success && pricingData.data) {
          setGlobalNotes({
            ...
            transportNote: pricingData.data.transportNote || ''
          });
          setGlobalPricing({
  ...
  const handleTabChange = (tId, e) => {
    ...
    if (window.innerWidth < 1024 && contentRef.current) {
      ...
      if (window.lenis) window.lenis.scrollTo(y, { duration: 1.2 });
  ...
  return (
    <div className="relative min-h-[100dvh] w-full overflow-x-hidden text-white bg-[#022c22] selection:bg-[#D9A441] selection:text-black">
      ...
      {/* FIXED: DYNAMIC SEO FROM BACKEND */}
      {/* FIXED: DYNAMIC SEO FROM BACKEND */}
      <SEO
        title={seoData.title}
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className="max-w-[1400px] mx-auto px-4 sm:px-6 pb-32 lg:pb-12 pt-28">
          <div className="flex flex-col lg:flex-row gap-12 items-start">
            ...
            <div className="flex-1 min-w-0 w-full">
              ...
              {/* 2. TITLE SECTION (MODIFIED: Location moved back) */}
              <div className="mt-12">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-8">
                  {/* --- LOCATION BADGE (Back to Old Style) --- */}
                ...
                <div className="grid grid-cols-3 gap-5 mt-10">
                  {[
                    ...
                    },
                    {
                      icon: <FaCalendarAlt />,
                      label: "Duration",
                      value: `${tour.nights}N / ${tour.nights + 1}D`
                    },
                    {
                      icon: <FaUsers />,
                      label: "Type",
                      ...
                      {activeTab === "overview" && (
                        <motion.div key="overview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.4 }}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-10">
                            <div className="bg-emerald-900/10 p-8 rounded-[2rem] border border-emerald-500/10"><h3 className="text-lg font-montserrat font-semibold text-emerald-400 mb-5 flex items-center gap-2"><FaCheckCircle /> Included</h3><ul className="space-y-4">{tour.inclusions?.map((item, i) => (<li key={i} className="flex items-start gap-3 text-emerald-100/80 text-sm font-inter font-light"><span className="text-emerald-400 mt-0.5"><FaCheckCircle /></span> {item}</li>))}</ul></div>
                            <div className="bg-red-900/10 p-8 rounded-[2rem] border border-red-500/10"><h3 className="text-lg font-montserrat font-semibold text-red-400 mb-5 flex items-center gap-2"><FaTimesCircle /> Excluded</h3><ul className="space-y-4">{["Personal Expenses", "Airfare / Train Tickets", "Lunch", "Room Heater Charges", "Entry Fees", "Additional Sight Seeing other than mentioned in itinerary", "Any Cost arising due to natural calamities-Lanslide,Political Disturbances(strikes), etc.to be borne by the client, which is directly payable on the spot."].map((item, i) => (<li key={i} className="flex items-start gap-3 text-gray-400 text-sm font-inter font-light"><span className="text-red-500 mt-0.5"><FaTimesCircle /></span> {item}</li>))}</ul></div>
                          </div>

                          {/* --- IMPORTANT DOCUMENTS SECTION --- */}
                          <div className="mt-10 bg-blue-900/10 p-6 md:p-8 rounded-[2rem] border border-blue-500/20">
                            <h3 className="text-lg font-montserrat font-semibold text-blue-400 mb-4 flex items-center gap-2">
                              <FaIdCard className="text-xl" /> Required Documents (Carry Original)
                            </h3>
                            <ul className="space-y-3">
                              <li className="flex items-start gap-3 text-blue-100/80 text-sm font-inter font-light">
                                <span className="text-blue-400 mt-1.5 text-[6px] shrink-0">●</span>
                                <span>Valid ID proof (Voter ID / Passport / Driving License)</span>
                              </li>
                              <li className="flex items-start gap-3 text-blue-100/80 text-sm font-inter font-light">
                                <span className="text-blue-400 mt-1.5 text-[6px] shrink-0">●</span>
                                <span>Birth Certificate / Passport (for children below 18 years)</span>
                              </li>
                              <li className="flex items-start gap-3 text-blue-100/80 text-sm font-inter font-light">
                                <span className="text-blue-400 mt-1.5 text-[6px] shrink-0">●</span>
                                <span>Passport-sized photographs for all guests (at least 4 copies)</span>
                              </li>
                            </ul>
                          </div>

                          {globalNotes.transportNote && (
                            <div className="mt-10 bg-red-900/10 p-6 md:p-8 rounded-[2rem] border border-red-500/20">
                              <h3 className="text-lg font-montserrat font-semibold text-yellow-400 mb-4 flex items-center gap-2">
                                <FaBus className="text-xl" /> Transport Policy
                              </h3>
                              {renderNotesWithBullets(globalNotes.transportNote)}
                            </div>
                          )}
                        </motion.div>
                      )}
                      ...
                      {activeTab === "food" && (
                        <motion.div key="food" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-12">
                          <div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{[{ icon: <FaUsers />, text: "Triple/Quad Sharing" }, { icon: <FaWifi />, text: "Free WiFi" }, { icon: <FaShower />, text: "Hot Water" }, { icon: <FaMountain />, text: "Scenic View" }].map((am, k) => (<div key={k} className="flex flex-col items-center justify-center text-center bg-white/5 p-6 rounded-[2rem] border border-white/5 backdrop-blur-sm"><span className="text-[#D9A441] mb-3 text-2xl">{am.icon}</span><span className="text-sm text-gray-200 font-inter font-light whitespace-nowrap">{am.text}</span></div>))}</div>
                          </div>
                                    ...
                                    <div key={idx} className="flex items-center gap-4 bg-white/5 p-4 rounded-2xl border border-white/5 hover:bg-white/10 transition">
                                      ...
                                      <div className="flex-1">
                                        <div className="flex flex-wrap gap-2">
                                          {meals.length > 0 ? (
                                            meals.map((m, i) => (<span key={i} className="text-[10px] font-bold text-black bg-[#D9A441] px-2 py-1 rounded-md tracking-wide font-montserrat uppercase">{m}</span>))
