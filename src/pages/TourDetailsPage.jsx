...
export default function TourDetailsPage() {
  ...
  const faqs = tour.faqs?.length > 0 ? tour.faqs : [
    { q: "Is this suitable for families?", a: "Yes, this is a family-friendly tour with comfortable stays." },
    { q: "Are permits included?", a: "Yes, all inner-line permits are arranged by us." }
  ];
  ...
  return (
    <div className="relative min-h-screen w-full overflow-x-hidden text-white bg-[#102A43]">
      ...
      <style>{`
        ...
        .force-horizontal-scroll::-webkit-scrollbar { display: none !important; }
        .scroll-item { flex: 0 0 auto !important; }
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          ...
          <div className="w-full lg:w-2/3">
            ...
            <div className="mt-8">
              ...
              <div className="flex flex-wrap items-center gap-4 mt-5 text-base md:text-lg">
                ...
                <div className="flex items-center gap-2 text-[#D9A441] bg-white/5 px-3 py-1 rounded-full border border-white/10">
                  ...
                  <span className="text-gray-400 text-sm font-normal ml-1">(120+ Reviews)</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mt-8">
              <div className="p-4 rounded-xl bg-white/5 border border-white/10 flex flex-col items-center justify-center text-center backdrop-blur-md hover:bg-white/10 transition">
                <FaRupeeSign className="text-[#D9A441] mb-2 text-2xl" />
              ...
              <div className="p-4 rounded-xl bg-white/5 border border-white/10 flex flex-col items-center justify-center text-center backdrop-blur-md hover:bg-white/10 transition">
                <FaCalendarAlt className="text-[#2E6F95] mb-2 text-2xl" />
              ...
              <div className="p-4 rounded-xl bg-white/5 border border-white/10 flex flex-col items-center justify-center text-center backdrop-blur-md hover:bg-white/10 transition">
                <FaUsers className="text-[#1F4F3C] mb-2 text-2xl" />
                <p className="text-[10px] sm:text-xs text-gray-400 uppercase tracking-wider font-bold">Type</p>
                <p className="text-sm sm:text-xl font-bold leading-tight text-white">Mixed Group</p>
              </div>
            </div>
            ...
            <div className="mt-12 w-full relative z-20">
              ...
              <div className="mt-8 p-6 md:p-8 bg-white/5 border border-white/10 rounded-[2rem] backdrop-blur-md shadow-2xl min-h-[300px]">
                      ...
                      >
                        ...
                        <div className="flex flex-col items-center">
                          ...
                          {i !== tour.itinerary.length - 1 && (
                            <div className="w-0.5 h-full bg-gradient-to-b from-[#D9A441] to-transparent opacity-30 mt-1 group-hover:opacity-60 transition" />
                          )}
                        </div>
                ...
                {activeTab === "food" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     ...
                     <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="bg-gradient-to-br from-white/10 to-white/5 p-6 rounded-2xl border border-white/10 shadow-lg h-full flex flex-col">
                        <div className="flex items-center gap-3 mb-6 border-b border-white/10 pb-4">
                        ...
                        <div className="space-y-3 overflow-y-auto custom-scroll pr-2 flex-1 max-h-[300px]">
                           {tour.itinerary?.map((day, idx) => (
                               <div key={idx} className="flex items-start gap-3 bg-black/20 p-3 rounded-xl border border-white/5 hover:border-white/20 transition">
                                   <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-xs font-bold text-[#D9A441] shrink-0">
                                      D{day.day}
                                   </div>
                                       ...
                                       <div className="flex flex-wrap gap-2">
                                           {day.meals && day.meals.length > 0 ? (
                                               day.meals.map((meal, mIdx) => (
                                                   <span key={mIdx} className="text-[10px] font-bold text-black bg-[#D9A441] px-2 py-1 rounded-md shadow-sm">
                                                       {meal}
                                                   </span>
                                               ))
                                           ) : (
                  ...
                  <div className="space-y-4">
                    {tour.reviews?.length > 0 ? (
                        tour.reviews.map((review, i) => (
                            <motion.div key={i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className="bg-white/5 p-5 rounded-2xl border border-white/10">
                                <div className="flex justify-between items-start mb-2">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center text-white font-bold">{review.name.charAt(0)}</div>
                                        <div>
                                            <h4 className="font-bold text-white text-sm">{review.name}</h4>
                                            <div className="flex text-yellow-400 text-xs mt-0.5">{[...Array(5)].map((_, k) => <FaStar key={k} className={k < review.rating ? "text-yellow-400" : "text-gray-600"} />)}</div>
                                        </div>
                                    </div>
                                    <span className="text-xs text-gray-500">{review.date}</span>
                                </div>
                                <p className="text-gray-300 text-sm leading-relaxed mt-3 pl-13">"{review.text}"</p>
                            </motion.div>
                        ))
                    ) : (
                        <div className="text-center py-16 text-gray-400 bg-white/5 rounded-2xl border border-white/10 border-dashed">
                           <div className="text-5xl mb-4 opacity-50">✨</div>
                           <p>No reviews yet. Be the first to experience this trip!</p>
                        </div>
                    )}
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    {faqs.map((faq, i) => (
                      <div key={i} className="bg-white/5 border border-white/10 rounded-xl overflow-hidden transition hover:bg-white/10">
                        <button onClick={() => toggleFaq(i)} className="w-full p-5 flex justify-between items-center text-left focus:outline-none">
                        ...
                        <AnimatePresence>
                          {openFaq === i && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden bg-black/20">
                              <div className="p-5 pt-0"><p className="text-gray-300 text-sm leading-relaxed pt-4 border-t border-white/10">{faq.a}</p></div>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </div>
