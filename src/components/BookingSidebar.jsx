...
const QuantityControl = ({ label, subLabel, icon: Icon, value, onChange, min = 0 }) => (
  <div className="space-y-1">
    ...
    <div className="flex items-center bg-white/5 rounded-xl border border-white/10 overflow-hidden backdrop-blur-sm h-11">
      <button
        onClick={() => onChange(Math.max(min, Number(value) - 1))}
        className="p-3 hover:bg-white/10 text-yellow-400 disabled:text-gray-600 disabled:hover:bg-transparent"
        disabled={Number(value) <= min}
      >
        <FaMinus size={10} />
      </button>
      <div className="flex-1 text-center font-bold text-white">{value}</div>
      <button
        onClick={() => onChange(Number(value) + 1)}
        className="p-3 hover:bg-white/10 text-yellow-400"
      >
        <FaPlus size={10} />
      </button>
    </div>
...
/* -------------------------------------------------------------------
   🔥🔥 MAIN COMPONENT WITH REVAMPED PC SIDEBAR UI 🔥🔥
------------------------------------------------------------------- */
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[999]">
        <div className="glass-effect p-4 rounded-t-3xl flex items-center gap-4 border-t border-white/20 bg-[#0f172a]/95 backdrop-blur-xl shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
          ...
          <button
            ...
            className="bg-gradient-to-r from-[#D9A441] to-[#F59E0B] text-black py-3 px-8 rounded-xl font-bold text-lg shadow-lg active:scale-95 transition"
          >
            Book Now
          </button>
        </div>
